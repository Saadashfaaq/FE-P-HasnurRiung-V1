...
@Component({
  selector: 'app-desktop-table-leave-permit-employee',
  standalone: true,
  imports: [SharedModule, MatIconModule, NgSelectModule],
  templateUrl: './desktop-table-leave-permit-employee.component.html',
  styleUrl: './desktop-table-leave-permit-employee.component.scss',
})

export class DesktopTableLeavePermitEmployeeComponent {
  ...
  GetAllApplicationFormsEmployee() {
    ...
    this.subs.sink = this._formLeaveService
      .GetAllApplicationFormsEmployee(filter, this.sortValue, pagination)
      .subscribe(
        (resp) => {
          if (resp && resp.length) {
